...
contract Reader is Governable {
    ...
    function getAmountOut(IVault _vault, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256, uint256) {
        ...
        {
            uint256 usdAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usdAmount = usdAmount.mul(10 ** USD_DECIMALS).div(10 ** tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usdAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usdAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _usdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _usdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
    ...
    function getVaultTokenInfoV2(address _vault, address _weth, uint256 _usdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
