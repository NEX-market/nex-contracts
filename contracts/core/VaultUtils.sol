...
contract VaultUtils is IVaultUtils, Governable {
    ...
    constructor(IVault _vault, INitManager _nitManager) {
        vault = _vault;
        nitManager = _nitManager;
    }

    function updateCumulativeFundingRate(address /* _collateralToken */, address /* _indexToken */) public override pure returns (bool) {
        return true;
    }
    ...
    function getBuyUsdFeeBasisPoints(address _token, uint256 _usdAmount) public override view returns (uint256) {
        return getFeeBasisPoints(_token, _usdAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), true);
    }

    function getSellUsdFeeBasisPoints(address _token, uint256 _usdAmount) public override view returns (uint256) {
        return getFeeBasisPoints(_token, _usdAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), false);
    }
